...
class product_template(models.Model):
    ...
    serial_prefix = fields.Char('Serial Prefix', required=True, copy=True)
...
class product_product(models.Model):
    ...
    categ_id = fields.Many2one('product.category','Internal Category', related='product_tmpl_id.categ_id')
...
class hr_timekeeping_sheet(models.Model):
    ...
    def button_done(self):
        ...
        for line in self.line_ids:
            if line.serial_ids:
                quants = []
                # need list of singletons
                for serial in line.serial_ids:
                    for quant in serial.quant_ids:
                        quants.append(quant)
